...
for pageindex in range(1,25):
    ...
    for each in teacherData:
        ...
        t_data['职位'] = each['prorank']
        t_data['系别'] = each['unit']
        t_data['出生日期'] = each['birthday']
        t_data['联系电话'] = each['contact']

        if each_url == 'javascript:void(0)' or re.search('banyong',each_url):
            continue
        try:
            html = requests.get(each_url,headers=headers).content.decode('utf8')
            #print(html)
            ...
            t_data['education_exp'] = ''
            for div in selector.xpath('/html/body/div[2]/div[3]/div[2]/div[2]/div[1]/div/div[2]//div'):
                if div.xpath('./div[1]'):
                    t_data['education_exp'] = t_data['education_exp'] + div.xpath('./div[1]/span/text()')[0] + '\n'
                if div.xpath('./div[2]'):
                    for span in div.xpath('./div[2]//span'):
                        t_data['education_exp'] = t_data['education_exp'] + span.xpath('string(.)') + ' | '
            t_data['工作经历'] = ''
